...
(async function () {  // https://www.bilibili.com/video/BV1154y1m7j1
    try {
        ...
        let buttonstart = await driver.wait(until.elementLocated(By.className('bilibili-player-iconfont-web-fullscreen-off')), 5000);
        await buttonstart.click()
        await driver.sleep(2000)
        const results = await driver.executeScript(function () {
            //调用JavaScript获取当前视频时长
            var result = window.player.getPlayerState().webFullScreen
            return result
        })
        if (results) {
            const owncase = `
            获取页面元素 page.$('.bilibili-player-iconfont-web-fullscreen-off'); 
            执行 click 事件
            断言取值：player.getPlayerState().webFullScreen
            ` + '结果为： ' + results
            console.log(owncase)
        } else {
