...
(async function () {  // https://www.bilibili.com/video/BV1154y1m7j1
    try {
        ...
        let buttonstart = await driver.wait(until.elementLocated(By.className('bilibili-player-video-btn-setting-left-repeat')), 5000);
        ...
        const results = await driver.executeScript(function () {
            ...
            var result = window.player.getPlayerState().repeat
            return result
        })
        if (results) {
            const owncase = `
