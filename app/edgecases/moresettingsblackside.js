...
(async function () {  // https://www.bilibili.com/video/BV1154y1m7j1
    try {
        ...
        await driver.sleep(2000)
        let buttonblack = await driver.wait(until.elementLocated(By.className('bilibili-player-video-btn-setting-right-others-content-blackside')), 5000);
        ...
        const results = await driver.executeScript(function () {
            ...
            var result = data.video_status.blackside_state
            return result
        })
        if (!results) {
            const owncase = `
            ...
            执行 hover 事件
            获取页面元素 page.$('.bilibili-player-video-btn-setting-right-others-content-blackside'); 
            执行 click 事件
            断言取值：sessionStorage.bilibili_player_settings.video_status.blackside_state
            ` + '结果为： ' + results
            console.log(owncase)
        } else {
