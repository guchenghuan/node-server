...
puppeteer.launch(
    ...
    try {
        ...
        await page
            .waitForSelector('.bilibili-player-video-btn-setting-right-others')
            .then(async () => {
                try {
                    const playtype = await page.$('.bilibili-player-video-btn-setting-right-others');
        ...
        data.video_status.blackside_state;
    `);
                    if (!result) {
                        const owncase = `
