...
puppeteer.launch(
    ...
    try {
        ...
        await page
            .waitForSelector('.bilibili-player-video-danmaku-setting-left-flag-title')
            .then(async () => {
                try {
                    const setting = await page.$('.bilibili-player-video-danmaku-setting-left-flag-title');
                    await setting.click({
                        delay: 1500
                    })
                ...
                `);
                    ...
                    断言取值：localStorage.bilibili_player_settings.block.aiblock
                    ` + '结果为： ' + result
                        console.log(owncase)
                    } else {
