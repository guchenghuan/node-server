...
puppeteer.launch(
    ...
    try {
        ...
        await page
            .waitForSelector('.bilibili-player-video-btn-setting-right-player-choose')
            .then(async () => {
                try {
                    const playtype = await page.$('.bilibili-player-video-btn-setting-right-player-choose');
                    let box = await playtype.boundingBox();
                    // console.log(box)
                    const x = box.x + (box.width / 2) + 10;
                    // console.log(x)
                    ...
                    // 播放器选择
                    const result = await page.evaluate('player.getPlayerState().playerType');
                    if (result === "flash") {
                        const owncase = `
