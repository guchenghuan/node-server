...
puppeteer.launch(
    ...
    try {
        ...
        await page
            .waitForSelector('.bilibili-player-video-danmaku-setting-left-area')
            .then(async () => {
                try {
                    const playtype = await page.$('.bilibili-player-video-danmaku-setting-left-area');
        ...
        data.setting_config.danmakuArea;
    `);
        ...
        断言取值：localStorage.bilibili_player_settings.setting_config.danmakuArea;
        ` + '结果为： ' + result
                        console.log(owncase)
                    } else {
