...
puppeteer.launch(
    ...
    try {
        ...
        await page
            .waitForSelector('.bilibili-player-video-btn-setting-right-playtype')
            .then(async () => {
                try {
                    const playtype = await page.$('.bilibili-player-video-btn-setting-right-playtype');
    ...
    `);
        ...
        断言取值：sessionStorage.bilibili_player_settings.video_status.playtype
        ` + '结果为： ' + result
                        console.log(owncase)
                    } else {
