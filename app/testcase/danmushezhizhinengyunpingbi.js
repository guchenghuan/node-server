...
puppeteer.launch(
    ...
    try {
        ...
        await page
            .waitForSelector('.bilibili-player-video-danmaku-setting-left-flag-content')
            .then(async () => {
                try {
                    const playtype = await page.$('.bilibili-player-video-danmaku-setting-left-flag-content');
    ...
    `);
        ...
        断言取值：localStorage.bilibili_player_settings.block.ailevel;
        ` + '结果为： ' + result
                        console.log(owncase)
                    } else {
